...
parser.add_argument("-ua", "--randuseragents", action="store_true")
parser.add_argument("--sleeptime", default=1, type=int)
...
def init_socket(ip):
    ...
    send_header(sock, "User-Agent", random.choice(user_agents) if args.randuseragents else user_agents[0])
...
while True:
    ...
    for sock in list(list_of_sockets):
        try:
            send_header(sock, "X-a", random.randint(1, 5000))
        except socket.error:
